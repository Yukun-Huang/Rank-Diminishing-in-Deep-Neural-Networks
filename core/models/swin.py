...
@register_model
def swin_tiny_patch4_window7_224(pretrained=False, args=None, **kwargs):
    """ Swin-T @ 224x224, trained ImageNet-1k
    """
    model_kwargs = dict(
        patch_size=4, window_size=7, embed_dim=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24), args=args, **kwargs)
    model = _create_swin_transformer('swin_tiny_patch4_window7_224', pretrained=False, **model_kwargs)
    if pretrained:
        weight_path = osp.join(args.weight_dir, 'swin_tiny_patch4_window7_224.pth')
...
def SWIN(method, model_name, args=None, **kwargs):
    model = __factory[model_name](args=args, **kwargs)
    return model
