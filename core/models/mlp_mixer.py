...
from timm.models.layers import PatchEmbed, Mlp, GatedMlp, DropPath, lecun_normal_, to_2tuple
...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        x += self.drop_path(self.ls1 * self.linear_tokens(self.norm1(x).transpose(1, 2)).transpose(1, 2))
        x += self.drop_path(self.ls2 * self.mlp_channels(self.norm2(x)))
        return x
...
class MlpMixer(nn.Module):
    ...
    def __init__(
        ...
        self.layers = ['Layer{}'.format(i) for i in range(24)]
...
def gmixer_24_224(pretrained=False, **kwargs):
    ...
    if pretrained:
        weight_path = osp.join(kwargs['args'].weight_dir, 'gmixer_24_224_raa-7daf7ae6.pth')
...
def resmlp_24_224(pretrained=False, **kwargs):
    ...
    if pretrained:
        weight_path = osp.join(kwargs['args'].weight_dir, 'resmlp_24_224_raa-a8256759.pth')
        load_checkpoint(model, weight_path)
        print(f'Load weights from {weight_path}')
    return model
